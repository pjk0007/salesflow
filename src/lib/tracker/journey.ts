...
/**
 * 리드 여정 조회·조립. journey route의 몸통을 그대로 옮긴 것이며 웹과 MCP가 공유한다.
 * route에 있던 쿼리·분기를 한 글자도 바꾸지 않았다 — 여기서 결과가 달라지면
 * 기존 여정 화면이 조용히 틀린 내용을 보여준다.
 */
...
export interface JourneyArgs {
    recordId: number;
    orgId: string;
    /** business | tracker | email. 비면 전부 */
    channelFilter?: string[];
    /** ISO timestamp. null이면 제한 없음 */
    fromTs?: number | null;
    toTs?: number | null;
    /** 연결된 visitor를 통해 다른 레코드까지 통합할지. 기본 true */
    merge?: boolean;
}
...
export interface JourneyResult {
    ...
    nextActions: ReturnType<typeof buildNextActions>;
}

/** 레코드를 못 찾으면 null. org 격리는 호출부가 아니라 여기서 한다. */
export async function buildRecordJourney(args: JourneyArgs): Promise<JourneyResult | null> {
    const { recordId, orgId } = args;
    const channelFilter = args.channelFilter ?? [];
    const fromTs = args.fromTs ?? null;
    const toTs = args.toTs ?? null;
    ...
    const [record] = await db
        .select()
        .from(records)
        .where(and(eq(records.id, recordId), eq(records.orgId, orgId)));
    ...
    if (merge) {
        ...
        const repVisitors = await db
            ...
            .where(and(eq(trackerVisitors.recordId, recordId), eq(trackerVisitors.orgId, orgId)));
        ...
        if (vIds.length) {
            ...
            const orgRecords = await db
                ...
                .from(records)
                .where(and(inArray(records.id, candidateIds), eq(records.orgId, orgId)));
    ...
    const [bizEvents, visitors, sendLogs] = await Promise.all([
        ...
        want("tracker")
            ? db.select({ id: trackerVisitors.id }).from(trackerVisitors).where(
                and(inArray(trackerVisitors.recordId, recordIds), eq(trackerVisitors.orgId, orgId))
            )
            : Promise.resolve([]),
        want("email")
            ? db.select().from(emailSendLogs).where(
                and(inArray(emailSendLogs.recordId, recordIds), eq(emailSendLogs.orgId, orgId))
            )
            : Promise.resolve([]),
    ]);
    ...
    return { summary, events: filtered, attribution, nextActions };
}
