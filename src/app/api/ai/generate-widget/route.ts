...
import { getAiClient, generateWidget, checkTokenQuota, updateTokenUsage, logAiUsage } from "@/lib/ai";
...
export async function POST(req: NextRequest) {
    ...
    try {
        const result = await generateWidget(client, {
            prompt: prompt.trim(),
        ...
        return NextResponse.json({
            ...
            data: {
                ...
                stackByColumn: result.stackByColumn,
            },
        });
    } catch (error) {
        console.error("AI widget generation error:", error);
        const message = error instanceof Error ? error.message : "AI 위젯 생성에 실패했습니다.";
