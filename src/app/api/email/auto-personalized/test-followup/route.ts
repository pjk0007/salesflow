...
import { db, emailAutoPersonalizedLinks, emailSendLogs, records } from "@/lib/db";
 ...
 * body: { linkId, parentLogId, stepIndex, isOpened, testEmail?, mode: "preview" | "send" }
...
export async function POST(req: NextRequest) {
    ...
    if (!linkId || !parentLogId || stepIndex == null || isOpened == null) {
        return NextResponse.json(
            { success: false, error: "linkId, parentLogId, stepIndex, isOpened가 필요합니다." },
            { status: 400 }
        );
    }
    ...
    const result = await generateAiFollowupPreview(
        { linkId, parentLogId, stepIndex, isOpened },
        user.orgId
    );
