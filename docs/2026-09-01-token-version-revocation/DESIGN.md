...
- **🟡 UI가 옛 role을 계속 보여준다.** `/api/auth/me`가 JWT의 role을 반환하므로(`...user` 스프레드) 강등되면 관리자 메뉴가 남고 누르면 401인데 **클라이언트에 401 처리가 없다**. 사용자는 "저장이 안 된다"만 겪는다. 후속 두 가지: (a) `/api/auth/me`가 `organization_members`에서 role을 다시 읽어 반환 — **이미 그 테이블을 조회 중이라 추가 쿼리 0, 3줄**, (b) 401 시 로그아웃 유도. **(a)를 이번 범위에 넣을지 사용자 판단 필요.**
